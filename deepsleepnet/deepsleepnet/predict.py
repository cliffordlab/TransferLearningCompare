...
def predict(
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        ...
        checkpoint_path = output_dir + '/deepsleepnet'
        saver = tf.compat.v1.train.Saver()
        saver.restore(sess, tf.train.latest_checkpoint(checkpoint_path))
        print("Model restored from: {}\n".format(tf.train.latest_checkpoint(checkpoint_path)))
        
        print("Testing on the following files: " + str(dataFileList))
        y_pred_ = []
        y_true_ = []
        for dataFile in dataFileList:

            # Load testing data
            x, y = SeqDataLoader.load_subject_data(dataFile,dataset)
            ''',
            ...
            pt_y_true_, pt_y_pred_, loss, duration = \
                custom_run_epoch(
                    sess=sess, network=valid_net,
                    inputs=[x[0].reshape(x[0].shape[:-1])], targets=y,
                    train_op=tf.no_op(),
                    is_train=False,
                    output_dir=output_dir) #,subject_idx=subject_idx)

            # Add predictions and groundtruths from individual patients to larger list
            y_true_.append(pt_y_true_)
            y_pred_.append(pt_y_pred_)
            
        # Dispaly Confusion Matrix
        y_true_ = np.concatenate(y_true_)
        y_pred_ = np.concatenate(y_pred_)
        np.set_printoptions(suppress=True)
        print(100*confusion_matrix(y_pred_,y_true_,normalize='true').T)
        np.set_printoptions(suppress=False)
            
        # Calculate Performance Metrics
        acc = accuracy_score(y_pred_,y_true_)
        prec = precision_score(y_pred_,y_true_,average='macro')
        rec = recall_score(y_pred_,y_true_,average='macro')
        f1 = f1_score(y_pred_,y_true_,average='macro')
        kappa = cohen_kappa_score(y_pred_,y_true_)
