...
class DeepFeatureNetTrainer(Trainer):
    ...
    def train(self, n_epochs, resume):
        with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
            ...
            output_dir = os.path.join(self.output_dir, train_net.name) #os.path.join(self.output_dir, "fold{}".format(self.fold_idx), train_net.name)
            ...
            for epoch in range(sess.run(global_step), n_epochs):
                ...
                np.savez(
                    os.path.join(output_dir, "perf.npz"), #os.path.join(output_dir, "perf_fold{}.npz".format(self.fold_idx)),
                ...
                if ((epoch + 1) % self.interval_save_model == 0) or ((epoch + 1) == n_epochs):
                    ...
                    save_path = os.path.join(
                        output_dir, "checkpoint.cpkt" #"model_fold{}.ckpt".format(self.fold_idx)
                    )
                ...
                if ((epoch + 1) % self.interval_save_model == 0) or ((epoch + 1) == n_epochs):
                    ...
                    np.savez(
                        os.path.join(
                            output_dir,
                            "params.npz"), #"params_fold{}.npz".format(self.fold_idx)),
                        **save_dict
                    )
                    duration = time.time() - start_time
        ...
        return os.path.join(output_dir, "params.npz") #"params_fold{}.npz".format(self.fold_idx))
...
class DeepSleepNetTrainer(Trainer):
    ...
    def get_intermediate_out(self, sess, network, inputs, targets, layer):
        start_time = time.time()
        layerOuputs = []
        for sub_idx, each_data in enumerate(zip(inputs, targets)):
            each_x, each_y = each_data
            ...
            for x_batch, y_batch in iterate_batch_seq_minibatches(inputs=each_x,
                ...
                batchLayerOutput = sess.run(
                    [layer],
                    feed_dict=feed_dict
                )

                layerOuputs.append(batchLayerOutput)

        return np.hstack(layerOuputs)
    ...
    def finetune(self, pretrained_model_path, n_epochs, resume):
        ...
        with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
            ...
            if load_pretrain:
                ...
                with np.load(pretrained_model_path) as f:
                    for k, v in f.items():
                        ...
                        #tmp_tensor = tf.Variable([1], shape=tf.TensorShape(None))
                        sess.run(
                            tf.compat.v1.assign(
                                tmp_tensor, 
                                v
            ...
            for epoch in range(sess.run(global_step), n_epochs):
                ...
                # Save performance history
                np.savez(
                    os.path.join(output_dir, "perf.npz"), #"perf_fold{}.npz".format(self.fold_idx)),
                    train_loss=all_train_loss, valid_loss=all_valid_loss,
                    train_acc=all_train_acc, valid_acc=all_valid_acc,
                    train_f1=all_train_f1, valid_f1=all_valid_f1,
                    y_true_val=np.asarray(y_true_val),
                ...
                if ((epoch + 1) % self.interval_save_model == 0) or ((epoch + 1) == n_epochs):
                    start_time = time.time()
                    save_path = os.path.join(
                        output_dir, "model.ckpt" #model_fold{}.ckpt".format(self.fold_idx)
                    )
                    saver.save(sess, save_path, global_step=global_step)
                    duration = time.time() - start_time
                    print("Saved model checkpoint ({:.3f} sec)".format(duration))
                ...
                if ((epoch + 1) % self.interval_save_model == 0) or ((epoch + 1) == n_epochs):
                    start_time = time.time()
                    save_dict = {}
                    if ((epoch + 1) == n_epochs): kerasMdl = deepSleepNet(inputShape = (25,x_train[0].shape[1])+x_train[0].shape[-1:]) # keras version of deepsleepnet
                    for v in tf.compat.v1.global_variables():
                        save_dict[v.name] = sess.run(v)
                        if ((epoch + 1) == n_epochs):
                            # On final training epoch, load all weights into keras model
                            if 'deepsleepnet' not in v.name: continue # Skip variables not included in keras model
                            
                            # Get name of layer and parameter name of each parameter. Note that some
                            # strings are replaced to account for changes in parameter names with keras
                            _,layerName,varName = re.split(r'^([\w]+/[\w]+)',v.name[13:] \
                                                           .replace('biases','bias') \
                                                           .replace('weights:','kernel:') \
                                                           .replace('l24_softmax_linear','l24_softmax_linear/fc'))
                            
                            if layerName in [layer.name for layer in kerasMdl.layers] and 'Adam' not in varName:
                                # If the parameter is one of the parameters in the keras version of the architecture,
                                # load the parameter value into the keras model
                                param = [weight for weight in kerasMdl.get_layer(layerName).weights if weight.name == (layerName + varName) or (layerName == 'l21_bi_lstm/bidirectional_rnn' and '/' + weight.name == varName)][0]
                                print('Setting value of ' + param.name)
                                tf.compat.v2.keras.backend.set_value(param,sess.run(v).reshape(param.shape))
                    ...
                    if ((epoch + 1) == n_epochs):
                        ...
                        kerasMdl.save_weights(output_dir + '/kerasMdl/model')
                    duration = time.time() - start_time
                    print("Saved trained parameters ({:.3f} sec)".format(duration))
        
        print("Finish fine-tuning")
        '''
        tf.saved_model.simple_save(sess,output_dir + '/kerasMdl',
                                   {valid_net.input_var.name:valid_net.input_var,valid_net.target_var.name:valid_net.target_var},
                                   {valid_net.pred_op.name:valid_net.pred_op})'''
        
        return (os.path.join(output_dir, "params.npz"),output_dir + '/kerasMdl/model')
