...
def perf_overall(dataFileList):
    # Remove non-output files, and perform ascending sort
    allfiles = os.listdir(dataFileList)
    outputfiles = []
    for idx, f in enumerate(allfiles):
        if re.match("^output_.+\d+\.npz", f):
            outputfiles.append(os.path.join(dataFileList, f))
    outputfiles.sort()
...
def main():
    ...
    parser.add_argument("--dataFileList", type=str, default="/home/akara/Workspace/deepsleep_output/results/outputs",
                        help="Directory where to load prediction outputs")
    args = parser.parse_args()

    if args.dataFileList is not None:
        perf_overall(dataFileList=args.dataFileList)
