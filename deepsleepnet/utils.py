...
def splitDataSequential(ptList,validationList,stages,earlyStoppingFraction=.1,seqLength = 25):
    ...
    removeUnknowns = lambda samples,labels: np.delete(samples,np.where(labels == stages['undefined']),axis=0)# Filter for removing samples of unknown sleep stage
    if validationList:
        # Create dataset from patients designated for use in validation, if any
        validationDataset = []
        for pt in validationList:
            # Make an individual dataset for each recording in the list of validation subjects
            validationData = np.array([removeUnknowns(ptList[pt]['features'],ptList[pt]['labels'])[seqIdx*seqLength:(seqIdx+1)*seqLength]
                                       for seqIdx in range(int(len(removeUnknowns(ptList[pt]['features'],ptList[pt]['labels']))/seqLength))])
            validationLabels = np.array([to_categorical(removeUnknowns(ptList[pt]['labels'],ptList[pt]['labels'])[seqIdx*seqLength:(seqIdx+1)*seqLength],len(stages)-1)
                                         for seqIdx in range(int(len(removeUnknowns(ptList[pt]['labels'],ptList[pt]['labels']))/seqLength))])
            validationDataset.append([validationData,validationLabels]) #validationDataset.append(tf.data.Dataset.from_tensor_slices((validationData,validationLabels)).batch(1))
        
            del validationData, validationLabels
    else:
        validationDataset = None
    
    # Get list of patients who aren't used for validation
    trainList = [pt for pt in range(len(ptList)) if pt not in validationList] 
    
    if earlyStoppingFraction != 0:
        # Create dataset for use in early stopping, if specified
        earlyStoppingList = trainList[::np.round(1/earlyStoppingFraction).astype(int)] # Select set of patients for use in early stopping
        trainList = [pt for pt in trainList if pt not in earlyStoppingList] # Remove early stopping set from training set
        
        earlyStoppingData = np.array([removeUnknowns(ptList[pt]['features'],ptList[pt]['labels'])[seqIdx*seqLength:(seqIdx+1)*seqLength]
                                      for pt in earlyStoppingList
                                      for seqIdx in range(int(len(removeUnknowns(ptList[pt]['features'],ptList[pt]['labels']))/seqLength))])
        earlyStoppingLabels = np.array([to_categorical(removeUnknowns(ptList[pt]['labels'],ptList[pt]['labels'])[seqIdx*seqLength:(seqIdx+1)*seqLength],len(stages)-1)
                                        for pt in earlyStoppingList
                                        for seqIdx in range(int(len(removeUnknowns(ptList[pt]['labels'],ptList[pt]['labels']))/seqLength))])
        earlyStoppingDataset = [earlyStoppingData,earlyStoppingLabels] #earlyStoppingDataset = tf.data.Dataset.from_tensor_slices((earlyStoppingData,earlyStoppingLabels)).batch(1)
        del earlyStoppingData, earlyStoppingLabels
    else:
        earlyStoppingDataset = None
        
    # Create dataset for training
    trainData = np.array([removeUnknowns(ptList[pt]['features'],ptList[pt]['labels'])[seqIdx*seqLength:(seqIdx+1)*seqLength]
                             for pt in trainList
                             for seqIdx in range(int(len(removeUnknowns(ptList[pt]['features'],ptList[pt]['labels']))/seqLength))])
    trainLabels = np.array([to_categorical(removeUnknowns(ptList[pt]['labels'],ptList[pt]['labels'])[seqIdx*seqLength:(seqIdx+1)*seqLength],len(stages)-1)
                               for pt in trainList
                               for seqIdx in range(int(len(removeUnknowns(ptList[pt]['labels'],ptList[pt]['labels']))/seqLength))])
    trainDataset = [trainData,trainLabels] #tf.data.Dataset.from_tensor_slices((trainData,trainLabels)).batch(1)
    
    # Obtain Class Weights
    weightLabels = trainLabels.flatten()
...
def transferDatasetAssembler(data,targetData,validationList,stages,**kwargs):
    ...
    if (('sequential' in kwargs) and kwargs['sequential']):
        # Using RNN, and performing transfer learning
        sourceDataset,_,_,_ = splitDataSequential(data,[],stages,earlyStoppingFraction = 0.0)
        targetDataset,earlyStoppingDataset,validationDataset,class_weights = splitDataSequential(targetData,validationList,stages)
...
def datasetAssembler(data,validationList,stages,**kwargs):
    ...
    if (('sequential' in kwargs) and kwargs['sequential']):
        ...
        trainDataset,earlyStoppingDataset,validationDataset,class_weights = splitDataSequential(data,validationList,stages)
...
def getDatasetShape(dataset, featureName = None, sequentialDataset = False):
    ...
    for _,batch in enumerate(dataset):
        ...
        sampsPerTimeSeries = (batch[0].shape[1] if sequentialDataset else None) if featureName is None else (batch[0][featureName].shape[1] if sequentialDataset else None) # Samples per patient, if data is sequential
        break
...
def getRawSignal(eeg,fs,downSampleTo = None,epochDur = 30):
    # Helper function which returns data as list of epochs of
    ...
    # Initialize dataset
    numEpochs = int(eeg.shape[1]/(fs*epochDur))
    dataset = [None]*numEpochs
    
    # Cycle through each epoch and extract data
    for epoch in range(numEpochs):
        for channel in range(eeg.shape[0]):
            # Obtain Features
            dataset[epoch] = eeg[channel,(epoch*fs*epochDur):((epoch+1)*fs*epochDur)]
            
    return np.array(dataset)[:,:,None]


def extractRecordsParallel(ExtractionFunc,dataFileList,*args,ignoreNumPts = False):
    # Helper function for parallelizing Features extraction code
    # 
    # INPUTS
    # ExtractionFunc -------------------- (function) Function to run on each file
    # dataFileList --------------------------- (string list) List of directories or files to extract from
    ...
    ptList = Parallel(n_jobs=num_cores)(delayed(ExtractionFunc)(record,epochDur,*args) for record in dataFileList) #[ExtractionFunc(record,epochDur,*args) for record in dataFileList] 
    
    assert ignoreNumPts or ((None not in ptList) and len(ptList) == len(dataFileList)), 'Unknown Error: fewer than expected number of patients in dataset'
...
def extractFromCicc_parallel(record,epochDur,downSampleTo,possibleChannels,stages,extractFeatures):
    ...
    # Sanity check
    datasetLength = dataset['stft'].shape[0] if dataset.__class__ is dict else len(dataset) if dataset.__class__ is list else dataset.shape[0]
...
def extractFromShhs_parallel(labelDir,record,epochDur,downSampleTo,possibleChannels,stages,extractFeatures):
    ...
    # Sanity check
    assert labels.shape[0] - len(dataset) == 0, 'Unexpectected mismatch in size between data and labels'
    
    return {'features':dataset,'labels':labels[0:len(dataset)],'patientId':record.split("/")[-1]}
...
def extractFromSSC_SSC_parallel(record,epochDur,downSampleTo,channel,stages,extractFeatures):
    ...
    if labels.shape[0] - len(dataset) == 1: labels = labels[0:-1]
...
def extractFromSSC_IS_RC_parallel(record,epochDur,downSampleTo,channel,stages,extractFeatures):
    ...
    # Extract Features
    dataset = extractFeatures(eeg,fs,downSampleTo = downSampleTo,epochDur = epochDur)
    labels = labels[0:len(dataset)] # Discard labels past point where EEG recording ends
...
def extractFromMrOs_parallel(record,epochDur,downSampleTo,possibleChannels,stages,extractFeatures):
    ...
    if (labels.shape[0] - len(dataset) != 0):
...
def extractFromEegBudsAnn_parallel(record,epochDur,downSampleTo,possibleChannels,stages,extractFeatures):
    ...
    print("Obtaining data from record " + record)
    subjectName = re.split('_sess_',re.split('(test_._|sleep_center_esc_)',record)[-1])[0]
    ...
    # Sanity check
    assert len(labels) - len(dataset) == 0, ('Unexpectected mismatch in size between data and labels for record ' + record)
...
def extractFromEegBudsRml_parallel(record,epochDur,downSampleTo,possibleChannels,stages,extractFeatures):
    ...
    if (labels.shape[0] > len(dataset)): labels = labels[:len(dataset)]
    if (labels.shape[0] < len(dataset)): dataset = dataset[:labels.shape[0]] 
...
def extractFromCicc(dataDir = None,downSampleTo = None,possibleChannels = ["C3-M2"],stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from 2018 Computing in Cardiology Challenge Dataset
    if dataFileList is None: dataFileList = glob(dataDir + '/*/')[:936:13]
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    print(str(len(dataFileList)) + ' subjects in source dataset.')
    return extractRecordsParallel(extractFromCicc_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromWsc(dataDir = None,downSampleTo = None,possibleChannels = [["C3_M2"]],stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from Wisconsin Sleep Cohort
    if dataFileList is None: dataFileList = glob(dataDir + '/*.edf')[:1080:15]
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    print(str(len(dataFileList)) + ' subjects in source dataset.')
    return extractRecordsParallel(extractFromWsc_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromShhs(dataDir = None,downSampleTo = None,possibleChannels = [["EEG(sec)","EEG2","EEG 2","EEG sec"]],stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from Sleep Heart Health Study
    if dataFileList is None: dataFileList = glob(dataDir + '/edfs/shhs1/*.edf')[:5544:77]
    if dataDir is None: dataDir = dataFileList[0].split('/edfs')[0] # Infer data directory from data file list in order to locate labels
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    print(str(len(dataFileList)) + ' subjects in source dataset.')
    return extractRecordsParallel(lambda *args: extractFromShhs_parallel(dataDir + '/labels/',*args),
                                  dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromSleepProfiler(dataDir = None,downSampleTo = None,possibleChannels = [["EEG3"]],stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from Sleep Profiler Dataset
    if dataFileList is None: dataFileList = glob(dataDir + '/*.edf')
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    return extractRecordsParallel(extractFromSleepProfiler_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromMass(dataDir = None,downSampleTo = None,possibleChannels = "C3",stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from MASS dataset
    if dataFileList is None: dataFileList = glob(dataDir + '/*/*PSG.edf')[:144:2]
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    print(str(len(dataFileList)) + ' subjects in source dataset.')
    return extractRecordsParallel(extractFromMass_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromIsruc(dataDir = None,downSampleTo = None,possibleChannels = ["C3-A2","C3-M2","C3"],stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from ISRUC dataset
    if dataFileList is None: dataFileList = (glob(dataDir + '/subgroupI/*/*.rec') + glob(dataDir + '/subgroupII/*/*1.rec') + glob(dataDir + '/subgroupIII/*/*1.rec'))[:72]
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    print(str(len(dataFileList)) + ' subjects in source dataset.')
    return extractRecordsParallel(extractFromIsruc_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)
    
def extractFromSSC_SSC(dataDir,downSampleTo,possibleChannels,stages,extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Helper function for extracting Features from SSC and CNC subdirectories of SSC dataset
    dataFileList = (glob(dataDir + '/SSC/*.EDF') + glob(dataDir + '/CNC/*.edf'))
    #dataFileList = dataFileList[::3] + dataFileList[1::3] #[0:6] # Skip every third file for holdout set
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    return extractRecordsParallel(extractFromSSC_SSC_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromSSC_IS_RC(dataDir,downSampleTo,possibleChannels,stages,extractFeatures=getSTFT,debug = False,dataFileList = None):
    ...
    dataFileList = glob(dataDir + '/IS-RC/*.edf')
    #dataFileList = dataFileList[::3] + dataFileList[1::3] #[0:6] # Skip every third file for holdout set
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    return extractRecordsParallel(extractFromSSC_IS_RC_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromSSC_DHC(dataDir,downSampleTo,possibleChannels,stages,extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Helper function for extracting Features from DHC subdirectory of SSC dataset
    dataFileList = (glob(dataDir + '/*/*/*/' + possibleChannels + '.mat') + glob(dataDir + '/*/*/' + possibleChannels + '.mat'))
    #dataFileList = dataFileList[::3] + dataFileList[1::3] #[0:6] # Skip every third file for holdout set
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    return extractRecordsParallel(extractFromSSC_DHC_parallel,dataFileList,downSampleTo,possibleChannels.replace('-',''),stages)
    
def extractFromSSC(dataDir,downSampleTo = None,possibleChannels = 'C3-A2',stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
...
def extractFromMrOs(dataDir = None,downSampleTo = None,possibleChannels = "C3",stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from MrOS dataset
    if dataFileList is None: dataFileList = glob(dataDir + '/polysomnography/edfs/visit1/*.edf')[:2880:40]
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    print(str(len(dataFileList)) + ' subjects in source dataset.')
    return extractRecordsParallel(extractFromMrOs_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromEegBudsAnn(dataDir = None,downSampleTo = None,possibleChannels = [['1']],stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from EEGBud Dataset for recordings with data stored in .ann
    if dataFileList is None: dataFileList = glob(dataDir + '/*.edf') #glob(dataDir + '/*/sess_?/*.edf') 
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    return extractRecordsParallel(extractFromEegBudsAnn_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromEegBudsRml(dataDir = None,downSampleTo = None,possibleChannels = [['1']],stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from EEGBud Dataset for recordings with labels stored in .rml files
    dataFileList = glob(dataDir + '/esc_???/data/sess_???/eeg') #[0:6]
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    return extractRecordsParallel(extractFromEegBudsRml_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)

def extractFromEegBuds(dataDir = None,downSampleTo = None,possibleChannels = [['1']],stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
...
def extractEegBudUnlabeled(dataDir = None,downSampleTo = None,possibleChannels = [['1']],stages = {'n3':0,'n2':1,'n1':2,'rem':3,'awake':4,'undefined':5},extractFeatures=getSTFT,debug = False,dataFileList = None):
    # Extracts Features from EEGBud Dataset for recordings with no labels
    if dataDir is None and dataFileList is None: raise Exception('Specify either a directory or list of files to extract from')
    if dataFileList is None: dataFileList = glob(dataDir + '/unlabeled/*.edf') + glob(dataDir + '/unlabeled/*.bdf')
    if debug: dataFileList = dataFileList[0:6] # Only use a few subjects during debugging
    return extractRecordsParallel(extractEegBudUnlabeled_parallel,dataFileList,downSampleTo,possibleChannels,stages,extractFeatures)
