...
from transfertools.models.sstca import SSTCA
from tensorflow.keras.models import Model
from tensorflow.keras.backend import function
from tensorflow.keras.utils import to_categorical
from tensorflow.keras import Sequential,losses
from tensorflow.keras.layers import Dense, Dropout, Input, BatchNormalization, Activation, Conv2D, MaxPool2D, Flatten, Reshape, LSTM, GlobalAveragePooling2D, TimeDistributed, Masking, Lambda, Concatenate
...
class WeightedTrainModel:
    ...
    def fit(self,sourceDataset, targetDataset, validation_data = None, weights = {"source":.9,"target":.1}, **kwargs):
        addSrcWeights = lambda x,y: (x,y,weights["source"]) #(x,y,tf.py_function(weights["source"],tf.float32))
        addTargWeights = lambda x,y: (x,y,weights["target"]) #(x,y,tf.py_function(weights["target"],tf.float32))
...
class CoralTransfer:
    ...
    def fit(self,sourceDataset, targetDataset, validation_data = None, **kwargs):
        ...
        self.mdlUpperLayers = Sequential(self.origMdl.layers[(index+1):]) # Make model from every subsequent layer
        ...
        extractActivations = lambda x,y: (tf.py_function(self.get_activations,[x],tf.float32),y)
        ...
        a_s_np,y_s_np = utils.dataset2Numpy(a_s_flat.shard(np.max([1,np.floor(numSourceSamps/numTargetSamps)]),0))
        
        # Ensure source and target dataset same size (required for domain transfer code)
        randomChoices = np.random.choice(a_s_np.shape[0],a_t_np.shape[0],replace=False)
        a_s_np = a_s_np[randomChoices]
        y_s_np = y_s_np[randomChoices]
        
        # Fit transform
        print('Training Domain Transformation')
        self.scaler.fit(a_t_np)
        self.transform.fit(Xs=a_s_np,Xt=self.scaler.transform(a_t_np),yt=np.argmax(y_t_np,axis=-1),ys=np.argmax(y_s_np,axis=-1))
        del a_s_np, a_t_np, y_s_np, y_t_np
        
        # Apply transform
        reverseReshape = lambda x,y: (tf.reshape(x,featureDims),y)
        transformer = lambda x,y: (tf.py_function(self.transform.transfer,[x,tf.math.argmax(y,axis=-1)],tf.float32),y)
        scaler = lambda x,y: (tf.py_function(self.scaler.transform,[x],tf.float32),y)
        a_t_transformed = a_t_flat.map(scaler,tf.data.experimental.AUTOTUNE).map(reverseReshape,tf.data.experimental.AUTOTUNE)
        a_s_transformed = a_s_flat.map(transformer,tf.data.experimental.AUTOTUNE).map(reverseReshape,tf.data.experimental.AUTOTUNE)
        ...
        if validation_data is not None:
            ...
            a_stopping = validation_data.map(extractActivations,tf.data.experimental.AUTOTUNE) \
                                         ...
                                         .map(scaler,tf.data.experimental.AUTOTUNE) \
        ...
        self.mdlUpperLayers.compile(optimizer = 'adam',loss = 'categorical_crossentropy',metrics = ['accuracy']) # NOTE: Will need to change if switch to RNN
    ...
    def predict(self,x):
        ...
        return self.mdlUpperLayers.predict(a_t_scaled)
...
class HeadRetrain:
    ...
    def fit(self,sourceDataset, targetDataset, **kwargs):
        ...
        for layer in self.origMdl.layers[:(index+1)]: layer.trainable = False
        self.origMdl.compile(optimizer = 'adam',loss = 'categorical_crossentropy',metrics = ['accuracy']) # NOTE: Will need to change if switch to RNN
        self.origMdl.fit(targetDataset,**kwargs)
    ...
    def splitModel(self,layerSignifier = None):
        ...
        # Get classifier
        upperLayers = Sequential(self.origMdl.layers[(index+1):]) # Make model from every subsequent layer
        classifier = lambda x: upperLayers.predict(np.reshape(x,(x.shape[0],) + layer.output_shape[1:]))
        
        return (featureExtractor,featureExtractor,classifier)
            
    def predict(self,x):
        return self.origMdl.predict(x)
...
class TcaTransfer:
    ...
    def fit(self,sourceDataset, targetDataset, validation_data = None, **kwargs):
        ...
        targetTransformer = lambda x,y: (tf.py_function(self.transform.transfer_target,[x],tf.float32),y)
...
class SubspaceAlignment:
    ...
    def fit(self,sourceDataset, targetDataset, validation_data = None, **kwargs):
        ...
        a_s_scaled = self.scaler_s.fit_transform(a_s_np)
        a_t_scaled = self.scaler_t.fit_transform(a_t_np)
        ...
        trainDataset = tf.data.Dataset.from_tensor_slices((np.concatenate([a_t_transformed,a_s_transformed]),
                                                           np.concatenate([y_t_np,y_s_np]))).batch(32)
        del a_t_transformed, a_s_transformed, y_s_np, y_t_np
        
        if validation_data is not None:
            # Also transform early stopping set, if provided
            a_stopping_flat = validation_data.map(extractActivations,tf.data.experimental.AUTOTUNE) \
                                             .unbatch() \
                                             .map(reshaper,tf.data.experimental.AUTOTUNE)
            a_stopping_np,y_stopping_np = utils.dataset2Numpy(a_stopping_flat)
            a_stopping_scaled = self.scaler_t.transform(a_stopping_np)
            a_stopping_transformed = self.pca_t.transform(a_stopping_scaled)

            a_stopping_dataset = tf.data.Dataset.from_tensor_slices((a_stopping_transformed,y_stopping_np)).batch(32)
            del a_stopping_flat, a_stopping_np, y_stopping_np, a_stopping_scaled, a_stopping_transformed
            
        # Retrain model on transformed features
        print('Retraining model')
        callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_accuracy', patience=30, min_delta = .001, restore_best_weights = True)] # Perform early stopping
        self.mdlUpperLayers.compile(optimizer = 'adam',loss = 'categorical_crossentropy',metrics = ['accuracy']) # NOTE: Will need to change if switch to RNN
        self.mdlUpperLayers.fit(trainDataset,validation_data=a_stopping_dataset,**kwargs)
