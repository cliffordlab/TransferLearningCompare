...
import tensorflow as tf
import utils
from models import CNN_2D, denseNet, FFN, CNN_LSTM, CoralTransfer, TcaTransfer, CNN_AdvInf, trainUnsupervized, AdvInfEarlyStopping, CNN_MultiInput, WeightedTrainModel, DeepDomainConfusion, WeightedSoftVote, SubspaceAlignment, HeadRetrain
...
def main(epochs = 1000,verbose = 0,**kwargs):
    ...
    for fold in range(numFolds):
        ...
        if kwargs['sequential']:
            ...
            datasetShape = (1,) + (sampsPerTimeSeries,) + featureShape
        else:
            if multiInput:
                stftShape = utils.getDatasetShape(trainDataset,'stft')[0]
                covShape = utils.getDatasetShape(trainDataset,'covariance')[0]
            else:
                datasetShape = (1,) + (utils.getDatasetShape(sourceDataset)[0] if (transfer or ensemble) else utils.getDatasetShape(trainDataset)[0] )
        ...
        if ('loadMdl' in kwargs) and (kwargs['loadMdl'] is not None):
            ...
            model = WeightedSoftVote((1,) + utils.getDatasetShape(sourceDataset)[0],
                                     kwargs['ensembleList'],
                                     kwargs['modelType'],
...
if (__name__ is "__main__") or (__name__ == "__main__"):
    ...
    transferMdl = homeDir + 'getShhsModel/model'
    targetFeatureExtraction = lambda *args,**kwargs: utils.extractFromSleepProfiler(*args,**kwargs,extractFeatures=extractFeatures,dirList = dirList,debug = (True if dataSet is 'debug' else False))
    ...
    main(epochs = epochs,verbose = verbose,saveMdl = logDir + 'model',logDir = logDir,dataLoadFunc = dataLoadFunc,saveDataTo=saveDataTo,loadDataFrom=loadDataFrom,sequential=sequential,
         max_trials = max_trials, dataDir = dataDir, singleFold = singleFold, holdOut = holdOut, numFolds = numFolds, tune = tune, loadMdl = loadMdl,retrain = retrain,transfer = transfer,
         ...
         unsupervisedPretrain = unsupervisedPretrain,unlabeledDataLoadFunc = unlabeledDataLoadFunc,unlabeledDataDir = unlabeledDataDir,multiInput = multiInput,ensemble = ensemble,ensembleList = ensembleList)
